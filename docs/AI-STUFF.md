...
```text
create/update the [AGENTS.md](https://agents.md) and use the best practices, think about how to optimize it. Use `rg` for file discovery, ensure CLAUDE.md is a symlink to AGENTS.md . Run and apply "npx -y @yawlabs/ctxlint --depth 5 --mcp --strict --fix --yes", "npx -y claudelint check-all --fix" and "npx -y agnix --fix ."
```
</details>
...
<details><summary>TODO->PLAN</summary>
...
3. Enrich each item:
   - infer intent from context (what was the author trying to do?)
   - identify blocking dependencies (does fixing X require changing Y first?)
   - classify: bug | performance | refactor | feature | security | debt | docs
4. Triage severity:
...
5. Build dependency graph: topological sort so agents can execute tasks in valid order
6. For each task generate:
   - one-line title (imperative, ≤72 chars)
   - file:line anchor
   - severity + category tags
   - acceptance criteria (2-5 bullet points, testable, no vague language)
...
```text
Apply all code suggestions in all pull requests. Merge the pr's together cleanly and intelligently into one pr, rebase it and squash merge it into main. Use mcp-servers and `git`/`gh` to accomplish the task. Delete the leftover branches after the merge and also delete any ghost branches.
```
</details>
