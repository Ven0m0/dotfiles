...
term_dim(){
  ...
  if [[ -n "${FZF_PREVIEW_TOP:-}" && -n "$dim_lines" ]]; then
    ...
    local tty_lines; tty_lines="$(stty size </dev/tty 2>/dev/null | awk '{print $1}')" || tty_lines="$lines"
...
preview_text(){
  local file="$1" center="${2:-0}" ext; ext="$(ext_of "$file")"
  case "$ext" in
    md) have glow && { glow --style=auto -- "$file"; return; } ;;
...
preview_symlink(){
  local loc="$1" target; target="$(readlink -- "$loc" || printf '')"
  [[ -z "$target" ]] && { printf 'symlink (unreadable)\n'; return; }
  printf 'symlink → %s\n' "$target"
}

preview_image_backend(){
  local img="$1" dim; dim="$(term_dim)"
...
preview_archive(){
  local f="$1" ext; ext="$(ext_of "$f")"
  case "$ext" in
    7z) have 7z && { 7z l -p -- "$f" || :; return; } ;;
    a|ace|alz|arc|arj|bz|bz2|cab|cpio|deb|gz|jar|lha|lz|lzh|lzma|lzo|rpm|rz|t7z|tar|tbz|tbz2|tgz|tlz|txz|tZ|tzo|war|xpi|xz|Z|zip|rar)
      have atool && { atool --list -- "$f" || :; return; } ;;
  esac
...
preview_misc_by_ext(){
  local f="$1" ext; ext="$(ext_of "$f")"
  case "$ext" in
    o) have nm && { nm -- "$f"; return; } ;;
    iso) have iso-info && { iso-info --no-header -l -- "$f"; return; } ;;
    odt|ods|odp|sxw) have odt2txt && { odt2txt -- "$f"; return; } ;;
    doc) have catdoc && { catdoc -- "$f"; return; } ;;
    docx) have docx2txt && { docx2txt -- "$f" -; return; } ;;
    xls|xlsx) if have ssconvert && have bat; then ssconvert --export-type=Gnumeric_stf:stf_csv -- "$f" "fd://1" | bat --language=csv; return; fi ;;
    wav|mp3|flac|m4a|wma|ape|ac3|og[agx]|spx|opus|as[fx]|mka) have exiftool && { exiftool -- "$f"; return; } ;;
  esac
  file --brief --dereference --mime -- "$f"
}
...
preview_file(){
  ...
  local mime; mime="$(mime_of "$loc" || printf '')"
  case "$mime" in
    ...
    video/*)
      local base hash out; base="$(abspath "$loc")"; hash="$(sha256_of "$base")"; out="${cache_dir}/thumb-${hash}.jpg"
      if ! [[ -s "$out" ]]; then have ffmpegthumbnailer && ffmpegthumbnailer -i "$loc" -o "$out" -s 1200 || :; fi
      [[ -s "$out" ]] && preview_image_backend "$out" || file -- "$loc"
      ;;
    application/pdf)
      local base hash out; base="$(abspath "$loc")"; hash="$(sha256_of "$base")"; out="${cache_dir}/pdf-${hash}.jpg"
...
parse_arg(){
  ...
  if [[ ! -r "$file" ]]; then
    if [[ "$file" =~ ^(.+):([0-9]+)\ *$ ]] && [[ -r "${BASH_REMATCH[1]}" ]]; then
      file="${BASH_REMATCH[1]}"; center="${BASH_REMATCH[2]}"
...
usage(){
  printf 'usage: %s preview <PATH|PATH:LINE>\n' "${0##*/}"
}
