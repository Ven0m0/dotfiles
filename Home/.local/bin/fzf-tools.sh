...
cmd_preview(){
  ...
  preview_file(){
    local loc="$1" center="${2:-0}"; local mime="$(mime_of "$loc" || printf '')"
    case "$mime" in
  ...
  local file center; read -r file center < <(parse_arg "$1")
...
cmd_git(){
  ...
  git_add(){
    _check_repo; local pager=$(_pager) files
  ...
  git_status(){
    _check_repo; local pager=$(_pager)
  ...
  git_branch(){
    _check_repo
    local branch=$(_git branch --all --color=always --sort=-committerdate "$@" | grep -v HEAD | _fzf_git \
  ...
  git_stash(){
    _check_repo; local pager=$(_pager) stash subcmd="${1:-}"
...
cmd_man(){
  local PREVIEW='man {1}'
  man -k . | "$FZF" --prompt='manual: ' --header='enter:open' \
    --delimiter=' ' --with-nth='1,2' \
    --preview="$PREVIEW"  --bind="enter:become:$PREVIEW"
}
# ============================================================================
# MAIN ROUTER
# ============================================================================
