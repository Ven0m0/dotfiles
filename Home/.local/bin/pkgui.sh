...
_pkgui_search_aur_rpc() {
  ...
  local count
  count=$(jq -r '.resultcount' <<< "$raw")
  ((count == 0)) && {
    _pkgui_warn "No results for '$query'"
    return 1
  }
  ...
  export -f _pkgui_fzf
  export FND FZF_THEME HIST PAC CACHE
  local selected
...
_pkgui_fw() {
  ...
  if fwupdmgr get-updates 2> /dev/null | grep -qE 'No updatable|No updates'; then
    echo "None"
  else
    fwupdmgr get-updates
    read -rp "Update?  [y/N] " ans
    [[ ${ans,,} == y ]] && fwupdmgr update
  fi
}
_pkgui_svc() {
  _pkgui_msg "Failed services..."
  local f
  f=$(systemctl --failed --no-pager --no-legend | wc -l)
  ((f > 0)) && systemctl --failed --no-pager || echo "None"
}
_pkgui_maint() {
  _pkgui_msg "Maintenance scan..."
  printf '\n%b=== Orphans ===%b\n' "$BD" "$D"
  "$PAC" -Qdttq 2> /dev/null | wc -l | xargs printf '%d\n'
  printf '\n%b=== Optional ===%b\n' "$BD" "$D"
  "$PAC" -Qettq 2> /dev/null | wc -l | xargs printf '%d\n'
  printf '\n%b=== Cache ===%b\n' "$BD" "$D"
  du -sh /var/cache/pacman/pkg/ 2> /dev/null || echo "N/A"
  printf '\n%b=== Failed Svc ===%b\n' "$BD" "$D"
  systemctl --failed --no-pager --no-legend | wc -l | xargs printf '%d\n'
  printf '\n'
}
...
_pkgui_export() {
  _pkgui_msg "Exporting native... "
  pacman -Qqne > "$FILE_NATIVE"
  _pkgui_msg "Native: $FILE_NATIVE ($(wc -l < "$FILE_NATIVE"))"
  _pkgui_msg "Exporting AUR..."
  pacman -Qqme > "$FILE_AUR"
  _pkgui_msg "AUR: $FILE_AUR ($(wc -l < "$FILE_AUR"))"
  printf '\n%bExport:%b\nNative:  %s\nAUR: %s\n' "$BD" "$D" "$FILE_NATIVE" "$FILE_AUR"
}
_pkgui_import() {
  [[ -s $FILE_NATIVE ]] && {
    _pkgui_msg "Importing native..."
    sudo pacman -S --needed - < "$FILE_NATIVE" || _pkgui_warn "Issues"
  } || _pkgui_warn "Skip native"
  if [[ -s $FILE_AUR ]]; then
    [[ $PAC == pacman ]] && _pkgui_warn "AUR needs paru/yay" || {
      _pkgui_msg "Importing AUR..."
      "$PAC" -S --needed - < "$FILE_AUR" || _pkgui_warn "Issues"
    }
...
_pkgui_menu() {
  ...
  while :; do
    choice=$(
      cat << 'MENU' | "$FND" --prompt='Action:  ' --height=40 --header="pkgui v5.3" || exit 0
s - Search packages
S - Search & install
l - List local
R - Remove packages
