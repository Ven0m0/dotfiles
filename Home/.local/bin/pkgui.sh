...
_vulns() {
	has arch-audit || {
		warn "Install: sudo pacman -S arch-audit"
		return 1
	}
...
_restore() {
	local b
	compgen -G "$CFG/backup-*.txt" >/dev/null || {
		warn "No backups"
		return 1
	}
