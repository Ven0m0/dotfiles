...
case $strategy in
  theirs)
    git merge "origin/$base_ref" -X theirs -m "Auto-merge: accept $base_ref" || {
      git checkout --theirs .; git add -A; git -c core.editor=true merge --continue; } ;;
  ours)
    git merge "origin/$base_ref" -X ours -m "Auto-merge: keep $head_ref" || {
      git checkout --ours .; git add -A; git -c core.editor=true merge --continue; } ;;
  auto)
    if git merge "origin/$base_ref" -m "Auto-merge: smart resolution"; then
      ...
      git add -A; git -c core.editor=true merge --continue
    fi ;;
