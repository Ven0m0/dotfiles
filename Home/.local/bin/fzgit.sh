...
_fzf(){
  ...
  while (($#)); do
    case "$1" in
      ...
      -p) opts+=(--preview "$2"); shift 2 ;;
      -w) opts+=(--preview-window "$2"); shift 2 ;;
...
case "${1,,}" in
  a|add) shift; _add "$@" ;;
  d|diff) shift; _diff "$@" ;;
  D|difftool) shift; _difftool "$@" ;;
  l|log) shift; _log "$@" ;;
  s|show) shift; _show "$@" ;;
  S|status) shift; _status "$@" ;;
  b|branch) shift; _branch "$@" ;;
  B|branches) shift; _branch_delete "$@" ;;
  t|tag) shift; _tag "$@" ;;
  c|commit) shift; _commit "$@" ;;
  r|revert) shift; _revert "$@" ;;
  R|reset) shift; _reset "$@" ;;
  f|file) shift; _file "$@" ;;
  st|stash) shift; _stash "$@" ;;
  sp|stashpush) shift; _stash_push "$@" ;;
  clean) shift; _clean "$@" ;;
  cp|cherry) shift; _cherry "$@" ;;
  rb|rebase) shift; _rebase "$@" ;;
  rl|reflog) shift; _reflog "$@" ;;
  bl|blame) shift; _blame "$@" ;;
  fx|fixup) shift; _fixup "$@" ;;
  sq|squash) shift; _squash "$@" ;;
  pr) shift; _pr "$@" ;;
  issue) shift; _issue "$@" ;;
  run) shift; _run "$@" ;;
  repo) shift; _repo "$@" ;;
  ig|ignore) shift; _ignore "$@" ;;
  clone) shift; _clone "$@" ;;
  -h|h|--help) _help ;;
  -v|v|--version) _ver ;;
