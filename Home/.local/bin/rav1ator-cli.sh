...
rav1e_link="https://raw.githubusercontent.com/ultimaxx/rav1ator-cli/main/bin/rav1e.zst"
x265_link="https://raw.githubusercontent.com/ultimaxx/rav1ator-cli/main/bin/x265.zst"
x264_link="https://raw.githubusercontent.com/ultimaxx/rav1ator-cli/main/bin/x264.zst"
svt_link="https://raw.githubusercontent.com/ultimaxx/rav1ator-cli/main/bin/SvtAv1EncApp.zst"
aom_link="https://raw.githubusercontent.com/ultimaxx/rav1ator-cli/main/bin/aomenc.zst"
legal_link="https://raw.githubusercontent.com/ultimaxx/rav1ator-cli/main/bin/legal.tar.zst"
...
version="0.4.0"
bin_latest_date="5 February 2025"
...
update_hash_check () {
 str=$(curl -s https://raw.githubusercontent.com/ultimaxx/rav1ator-cli/main/rav1ator-cli.md5)
 echo "$str"
}
...
if [[ "$*" == "-h" || "$*" == "--help" ]]; then
 ...
 echo -e " openssl ffmpeg python mkvtoolnix-cli vapoursynth gum numactl av1an ffms2"
 echo -e ""
 ...
 case $manualInstall in
  "Yes")
   ...
   if [ "$rav1e_sum" != "51da17ab528c13ea3b97f765e85965f97048308ccd8012e55a61e2ada38527d6" ]; then
    ...
    echo -e "${GREY}Please contact ultimaxx on GitHub.${RESET}"
    exit 1
   elif [ "$x265_sum" != "dd00527ea7b152a41bf1eb9cb89f8d8c4754afbdbe889fcce6fc9e24fab28c24" ]; then
    echo -e "${GREY}Bad binary integrity detected:${RESET} x265"
    echo -e "${GREY}Please contact ultimaxx on GitHub.${RESET}"
    exit 1
   elif [ "$x264_sum" != "3d4f9ddfd08370e76663756a3bb5ac4f78d47b7ce0da8498e3a87b2e26f7fc58" ]; then
    echo -e "${GREY}Bad binary integrity detected:${RESET} x264"
    echo -e "${GREY}Please contact ultimaxx on GitHub.${RESET}"
    exit 1
   elif [ "$svt_sum" != "78e33c4e06e65251b9ca6f7001d9c75369339dd7d77dfe3f247dc92851acbac5" ]; then
    echo -e "${GREY}Bad binary integrity detected:${RESET} SvtAv1EncApp"
    echo -e "${GREY}Please contact ultimaxx on GitHub.${RESET}"
    exit 1
   elif [ "$aom_sum" != "f3540fc7b24b8d0e8186081e1b9fbeb6dd0ba00a4ce09322ac47eaa52f7b496e" ]; then
...
param_select () {
 ...
 case $yn in
  ...
  "Default")
   case $encoder in
    ...
    "x265")
     ...
     ;;
    "aomenc")
     str="--good --bit-depth=10 --end-usage=q --threads=2 --dist-metric=qm-psnr --tune-content=psy101 --arnr-strength=2 --sharpness=2 --auto-tiling=4 --enable-keyframe-filtering=1 --lag-in-frames=48 --disable-kf --kf-max-dist=9999 --cpu-used=$speed --cq-level=$crf --denoise-noise-level=$grain --enable-dnl-denoising=0"
     ;;
    "SVT-AV1")
     str="--tune 3 --keyint -1 --lp 2 --crf "$crf" --preset "$speed" --film-grain "$grain""
     ;;
    "rav1e")
  ...
  "HDR10")
   case $encoder in
    "aomenc")
     str="--good --bit-depth=10 --end-usage=q --threads=2 --dist-metric=qm-psnr --tune-content=psy101 --arnr-strength=2 --sharpness=2 --auto-tiling=4 --enable-keyframe-filtering=1 --lag-in-frames=48 --disable-kf --kf-max-dist=9999 --cpu-used=$speed --cq-level=$crf --denoise-noise-level=$grain --enable-dnl-denoising=0 --color-primaries=bt2020 --transfer-characteristics=smpte2084 --matrix-coefficients=bt2020ncl"
     ;;
    "SVT-AV1")
     str="--tune 3 --keyint -1 --lp 2 --crf "$crf" --preset "$speed" --film-grain "$grain" --color-primaries 9 --transfer-characteristics 16 --matrix-coefficients 9"
     ;;
    "rav1e")
  ...
  "Animation")
   case $encoder in
    "aomenc")
     str="--good --bit-depth=10 --end-usage=q --threads=2 --tune-content=psy --arnr-strength=2 --enable-keyframe-filtering=1 --lag-in-frames=48 --auto-tiling=4 --disable-kf --kf-max-dist=9999 --dist-metric=qm-psnr --cpu-used=$speed --cq-level=$crf --denoise-noise-level=$grain --enable-dnl-denoising=0"
     ;;
    "SVT-AV1")
     str="--tune 2 --keyint -1 --lp 2 --crf "$crf" --preset "$speed" --film-grain "$grain""
     ;;
   esac
   ;;
 esac
...
chunkmethod_text () {
    str="The chunk method defines how Av1an splits up your video into smaller chunks for encoding. \nFFMS2 is faster and very reliable but can have issues with some sources. \nBestsource is slower but the most reliable. \nIf you're unsure, FFMS2 is recommended."
    echo "$str"
}
...
chunkmethod_select () {
 # Single threaded
 str=$(gum choose --limit=1 --cursor.foreground="161" --item.foreground="248" FFMS2 Bestsource)
 echo "$str"
}
  ...
  echo -e "${GREY}>${RESET} ${AV1RED}$encode_start${RESET}\n"
   ...
   "Start Batch")
    ...
    echo -e "ulimit -n 200000" > "rAV1ator-CLI_job${RAND}.sh"
       echo -e "find "$batch_dir" \( -name "\*.mkv" -o -name "\*.y4m" -o -name "\*.mp4" -o -name "\*.flv" -o -name "\*.webm" -o -name "\*.ivf" -o -name "\*.flv" -o -name "\*.yuv" \) -execdir av1an -i {} --verbose --sc-pix-format=yuv420p --split-method av-scenechange -m $chunk_string -c mkvmerge --sc-downscale-height $sc_downscale_res -e "$encoder_av1an" --force -v \"$params\" --pix-format yuv420p10le -f \"$ffmpegopts\" -a \"$audioopts\" --set-thread-affinity 2 -w $workers -o rAV1ator-CLI_job${RAND}/{}-encoded.mkv \;" >> "rAV1ator-CLI_job${RAND}.sh"
    echo -e "echo -e \"${BOLD}Encoding Finished!${RESET} Thanks for checking out ${BOLD}rAV1ator${RESET}${GREY}:${RESET} ${AV1RED}${BOLD}CLI${RESET} ${BOLD}Edition${RESET}${GREY}${BLINK}${BOLD}!${RESET}\"" >> "rAV1ator-CLI_job${RAND}.sh"

    # Handle history output to `.rav1ator-cli-history` text file
    echo "-------" >> ~/.rav1ator-cli-history
    date >> ~/.rav1ator-cli-history
    echo -e "av1an -i \"genericInput.mkv\" --verbose --sc-pix-format=yuv420p --split-method av-scenechange -m $chunk_string -c mkvmerge --sc-downscale-height $sc_downscale_res -e "$encoder_av1an" --force -v \"$params\" --pix-format yuv420p10le -f \"$ffmpegopts\" -a \"$audioopts\" --set-thread-affinity 2 -w $workers -o genericOutput.mkv" >> ~/.rav1ator-cli-history
    ...
    # The only difference between "Yes" and "Resume" is that this one has a resume flag. I don't think this should be the default
       echo -e "ulimit -n 200000 && av1an -i \"$INFILE\" --verbose --sc-pix-format=yuv420p --resume --temp \"rAV1ator-CLI_temp${RAND}\" --split-method av-scenechange -m $chunk_string -c mkvmerge --sc-downscale-height $sc_downscale_res -e "$encoder_av1an" --force -v \"$params\" --pix-format yuv420p10le -f \"$ffmpegopts\" -a \"$audioopts\" --set-thread-affinity 2 -w $workers -o \"$OUTFILE\"" > "rAV1ator-CLI_job${RAND}.sh"
       echo -e "echo -e \"${BOLD}Encoding Finished!${RESET} Thanks for checking out ${BOLD}rAV1ator${RESET}${GREY}:${RESET} ${AV1RED}${BOLD}CLI${RESET} ${BOLD}Edition${RESET}${GREY}${BLINK}${BOLD}!${RESET}\"" >> "rAV1ator-CLI_job${RAND}.sh"
   ...
   "Print")
    ...
    echo -e "${GREY}Generated script${RESET}${AV1RED}:${RESET} av1an -i \"$INFILE\" --verbose --sc-pix-format=yuv420p --split-method av-scenechange -m $chunk_string -c mkvmerge --sc-downscale-height $sc_downscale_res -e "$encoder_av1an" --force -v \"$params\" --pix-format yuv420p10le -f \"$ffmpegopts\" -a \"$audioopts\" --set-thread-affinity 2 -w $workers -o \"$OUTFILE\""

    # Handle history output to `.rav1ator-cli-history` text file
    echo "-------" >> ~/.rav1ator-cli-history
    date >> ~/.rav1ator-cli-history
    echo -e "av1an -i \"$INFILE\" --verbose --sc-pix-format=yuv420p --split-method av-scenechange -m $chunk_string -c mkvmerge --sc-downscale-height $sc_downscale_res -e "$encoder_av1an" --force -v \"$params\" --pix-format yuv420p10le -f \"$ffmpegopts\" -a \"$audioopts\" --set-thread-affinity 2 -w $workers -o \"$OUTFILE\"" >> ~/.rav1ator-cli-history
   ...
   "Print Batch")
    ...
    # Print the non-batch Av1an command to the terminal
       echo -e "${GREY}Generated script${RESET}${AV1RED}:${RESET} av1an -i \"genericInput.mkv\" --verbose --sc-pix-format=yuv420p --split-method av-scenechange -m $chunk_string -c mkvmerge --sc-downscale-height $sc_downscale_res -e "$encoder_av1an" --force -v \"$params\" --pix-format yuv420p10le -f \"$ffmpegopts\" -a \"$audioopts\" --set-thread-affinity 2 -w $workers -o \"genericOutput.mkv\""
