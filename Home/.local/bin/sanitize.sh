...
cmd_whitespace(){
  ...
  for f in "${files[@]}"; do
    ...
    if [[ $do_trailing -eq 1 ]]&&grep -q '[[:space:]]$' "$f"; then
      file_issues+=("trailing-ws"); sed_script+='s/[[:space:]]\+$//;'
    fi
    if [[ $do_cr -eq 1 ]]&&grep -q $'\r' "$f"; then
      file_issues+=("crlf"); sed_script+='s/\r//g;'
    fi
    if [[ $check -eq 0 && -n $sed_script ]]; then
      sed -i "$sed_script" "$f"; dirty=1
    fi
    if [[ $do_unicode -eq 1 ]]&&has perl; then
      if ! perl -ne 'exit 1 if /[\x{00A0}\x{202F}\x{200B}\x{00AD}]/' "$f"; then
        if [[ $check -eq 1 ]]; then
          ...
          perl -CS -0777 -i -pe 's/[\x{00A0}\x{202F}\x{200B}\x{00AD}]+/ /g' "$f"; dirty=1; file_issues+=("unicode")
        fi
      fi
    fi
...
cmd_filenames(){
  ...
  for src in "${files[@]}"; do
    [[ -e "$src" ]]||continue
    local dir base clean; dir=$(dirname "$src"); base=$(basename "$src"); clean="$base"
