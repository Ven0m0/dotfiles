...
PRESETS: Final = {
    "av1": Preset(
        ...
        (
            ...
            (
                "-svtav1-params",
                "tune=0:film-grain={grain}:enable-qm=1:qm-min=0:enable-variance-boost=1:tf-strength=1:sharpness=-2:tile-columns=1:tile-rows=0:enable-dlf=2:scd=1{fast_decode}",
            ),
            ("-c:a", "libopus"),
            ("-b:a", "{audio_bitrate}"),
            ("-ac", "{audio_channels}"),
...
class Log:
    ...
    def prog(self, cur: int, tot: int, fname: str) -> None:
        if not self.quiet:
            ...
            bar = (
                "█" * int(20 * cur / tot) + "░" * (20 - int(20 * cur / tot))
                if tot
                else "░" * 20
            )
            tot_str = str(tot) if tot else "?"
            print(
                f"\r[{bar}] {pct:5.1f}% ({cur}/{tot_str}) {fname[:40]:<40}",
                end="",
                flush=True,
            )
...
def build_filters(cfg: Config, is_video: bool) -> list[str]:
    ...
    if cfg.deinterlace and cfg.deinterlace != "off":
        ...
        if cfg.deinterlace in m:
            filters.append(m[cfg.deinterlace])
    if cfg.default_denoise or (cfg.denoise and cfg.denoise != "off"):
        if cfg.denoise == "nlmeans":
            h = {"ultralight": 2, "light": 4, "medium": 6, "strong": 8}.get(
    ...
    if cfg.deblock and cfg.deblock != "off":
        filters.append(f"deblock={cfg.deblock}")
    if cfg.rotate:
        rm = {90: "transpose=1", 180: "transpose=1,transpose=1", 270: "transpose=2"}
        if cfg.rotate in rm:
            filters.append(rm[cfg.rotate])
    if cfg.crop and cfg.crop != "off":
        filters.append(
            "cropdetect=24:16:0" if cfg.crop == "auto" else f"crop={cfg.crop}"
        )
    if cfg.scale:
        ...
        w, h = cfg.max_dim
        filters.append(
            f"scale='if(gte(iw,ih),min({w},iw),-2)':'if(gte(iw,ih),-2,min({h},ih))':flags=lanczos"
        )
    if cfg.default_deband:
...
def build_params(preset: Preset, cfg: Config) -> list[str]:
    ...
    fmt = {
        ...
        "keyint": str(cfg.keyint),
        "pix_fmt": cfg.pix_fmt,
        "audio_channels": str(cfg.audio_channels),
        "fast_decode": ":fast-decode=1" if cfg.fast_decode else "",
    }
    for k, v in preset.params:
        params.append(k)
        if v is not None:
            params.append(v.format(**fmt))
    params.extend(cfg.extra)
    return params


def run_ffmpeg(
    inp: Path, out: Path, params: list[str], quiet: bool, use_ffzap: bool
) -> bool:
    if use_ffzap and has("ffzap"):
        ...
        cmd = (
            [
                ...
                "-loglevel",
                "error",
                "-stats" if not quiet else "-nostats",
                "-i",
                str(inp),
            ]
            + params
...
def convert(
    inp: Path, out: Path, preset: Preset, cfg: Config, log: Log, retries: int = 3
) -> bool:
...
def gen_out_path(
    inp: Path, preset: Preset, cfg: Config, out_dir: Path | None, src_root: Path | None
) -> Path:
    fmt = {
        "crf": str(cfg.crf),
        "preset": str(cfg.preset),
    ...
    suffix = "".join(
        c for c in preset.suffix.format(**fmt) if c.isalnum() or c in "._-+"
    )
...
def process_item(
    ...
    if ok:
        ...
        return (
            stats,
            f"{inp.name} → {out.name}: {in_sz / 1e6:.2f}MB → {out_sz / 1e6:.2f}MB ({ratio:.1%})",
        )
    else:
        stats.failed += 1
...
def run_batch(
    ...
    log.info(
        f"Format: {preset.name} (.{preset.ext}), CRF {cfg.crf}, Preset {cfg.preset}, Grain {cfg.grain}"
    )
    if preset.is_video:
        log.info(f"Audio: {cfg.audio_bitrate}, {cfg.audio_channels}ch")
        filters: list[str] = []
        if cfg.max_dim:
            filters.append(f"max-dim={cfg.max_dim[0]}x{cfg.max_dim[1]}")
        if cfg.default_denoise:
            filters.append("denoise=hqdn3d:1.5:1.5:6:6")
        if cfg.deinterlace:
            filters.append(f"deinterlace={cfg.deinterlace}")
        if cfg.denoise:
            filters.append(f"denoise={cfg.denoise}:{cfg.denoise_strength}")
        if cfg.deblock:
            filters.append(f"deblock={cfg.deblock}")
        if cfg.default_deband:
            filters.append("deband")
        if cfg.rotate:
            filters.append(f"rotate={cfg.rotate}")
        if cfg.crop:
            filters.append(f"crop={cfg.crop}")
        if cfg.scale:
            filters.append(f"scale={cfg.scale}")
        if filters:
            log.info(f"Filters: {', '.join(filters)}")
...
""",
    ...
    p.add_argument(
        "--preset", type=int, default=3, help="SVT-AV1/VP9 preset (default: 3)"
    )
    p.add_argument(
        "--preset-name", default="slow", help="x264/x265 preset name (default: slow)"
    )
    ...
    v.add_argument(
        ...
        metavar=("W", "H"),
        default=[1920, 1080],
        help="Max dimensions (default: 1920 1080)",
    )
    v.add_argument(
        "--pix-fmt", default="yuv420p10le", help="Pixel format (default: yuv420p10le)"
    )
    v.add_argument(
        "--keyint", type=int, default=600, help="Keyframe interval (default: 600)"
    )
    v.add_argument(
        "--fast-decode", action="store_true", help="Enable SVT-AV1 fast-decode"
    )
    ...
    v.add_argument(
        "--audio-channels", type=int, default=2, help="Audio channels (default: 2)"
    )
    f = p.add_argument_group("filters")
    f.add_argument(
        "--deinterlace",
        choices=["off", "bwdif", "yadif", "decomb"],
        help="Deinterlace filter",
    )
    f.add_argument(
        "--denoise", choices=["off", "nlmeans", "hqdn3d"], help="Custom denoise filter"
    )
    f.add_argument(
        "--denoise-strength",
...
def main() -> int:
    ...
    cfg = Config(
        ...
        max_dim=tuple(args.max_dim),
        pix_fmt=args.pix_fmt,
        keyint=args.keyint,
        fast_decode=args.fast_decode,
        default_denoise=not args.no_denoise,
        default_deband=not args.no_deband,
            ...
            for pat in args.files:
                for p in glob.iglob(str(Path(pat).expanduser()), recursive=True):
                    ...
                    if path.is_file() and path.suffix.lower() in exts:
                        if (
                            args.format == "opus"
                            and path.suffix.lower() in PASSTHROUGH_EXTS
                        ):
                            continue
                        yield path
