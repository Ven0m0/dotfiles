...
readonly BLD=$'\e[1m' GRN=$'\e[32m' BLU=$'\e[34m' YLW=$'\e[33m' DEF=$'\e[0m'
log(){ printf '%b==>\e[0m %s\n' "${BLD}${BLU}" "$*";}
...
set_glthread(){ log "Profile: GLThread (OpenGL threading)";export __GL_THREADED_OPTIMIZATIONS="1" mesa_glthread="true";}
set_software(){ log "Profile: Software (CPU rendering)";export LIBGL_ALWAYS_SOFTWARE="1" __GLX_VENDOR_LIBRARY_NAME="mesa" VK_ICD_FILENAMES="/usr/share/vulkan/icd.d/lvp_icd.x86_64.json";}
...
PROFILES:
  sarek       Performance with DXVK/VKD3D tweaks (default)
  skth        Sarek variant (alias)
  glthread    OpenGL threading only
  software    Force CPU rendering
  default     No tweaks
EXAMPLES:
  gamerun sarek wine game.exe
...
main(){
  ...
  case $profile in
    sarek) set_sarek;;glthread) set_glthread;;software) set_software;;default) set_default;;
  esac
  while [[ ${1:-} =~ = ]];do log "Custom env: $1";export "$1";shift;done
