...
fmt_shell() {
  $dry_run || shfmt -w -i 2 -bn -ci -sr "$1"
  shellcheck "$1"
}
fmt_yaml() {
  local cfg=".yamlfmt"
  [[ ! -f $cfg ]] && cfg="$root/.config/yamlfmt/yamlfmt.yml"
  $dry_run || yamlfmt -conf "$cfg" "$1"
  yamllint -c "$root/.config/yamllint/config" -f parsable "$1"
}
fmt_python() {
  $dry_run || ruff format "$1"
  ruff check "$1"
}
fmt_markdown() { markdownlint "$1"; }
fmt_toml() { has taplo && { $dry_run || taplo fmt "$1"; }; }
fmt_lua() {
  has stylua && { $dry_run || stylua "$1"; }
  has selene && selene "$1"
}
...
fmt_yaml_min() {
  ...
  tmp=$(mktemp)
  if has yamlfmt; then
    yamlfmt -conf "$cfg" "$f" > "$tmp" 2> /dev/null || {
      rm -f "$tmp"
      return 1
    }
  else
    skip "$f" "yamlfmt missing"
    rm -f "$tmp"
    return 0
  fi
  out=$(wc -c < "$tmp")
