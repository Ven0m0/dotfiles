...
def get_repo_stats(repo: Path) -> dict:
  ...
  return {"commits": commits, "files": files, "age": age, "active": active, "authors": dict(authors)}
  ...
  try:
    for item in path.iterdir():
      if item.is_dir():
        if (item / ".git").is_dir():
          repos.append(item)
          print(f"Git repository found {item}")
        else:
...
def aggregate_stats(repos: list[Path], base: Path):
  """Aggregate stats from all repos and display."""
  if not repos:
    print("Git repository is not found in directory scan")
    return
...
if __name__ == "__main__":
  if len(sys.argv) != 2:
    sys.exit("Usage: git-summmary <directory>")
