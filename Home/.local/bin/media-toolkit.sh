...
usb_write() {
  ...
  grep -q "$dst" /proc/mounts && die "mounted: $dst"
  ...
  dd if="$iso" bs=4M status=none | pv --size "$sz" -pterb | sudo dd of="$dst" bs=4M status=none conv=fsync
  sync
  ok "Done"
}
iso2sd() {
...
pngzip() {
  ...
  while getopts ":gqtr:h" o; do case $o in g) GRAY=1 ;; q) VERB=0 ;; t) TOUCH=1 ;; r) DPI=${OPTARG//[^0-9]/} ;; h)
    ...
    exit 0
    ;;
  *) die "Invalid: -$OPTARG" ;; esac done
  shift $((OPTIND - 1))
  for c in pngquant oxipng; do req "$c"; done
  ((DPI > 0)) && req pngcrush
  local -a tgts=() fs
  for x in "${@:-.}"; do
    [[ -d $x ]] && {
      has fd && mapfile -t fs < <(fd -tf -e png "$x") || mapfile -t fs < <(find "$x" -type f -name '*.png')
      tgts+=("${fs[@]}")
    }
    [[ -f $x && $x == *.png ]] && tgts+=("$x")
  done
  [[ ${#tgts[@]} -eq 0 ]] && die "No PNGs"
  for f in "${tgts[@]}"; do
    [[ -f $f && -r $f && -w $f ]] || {
      ((VERB)) && printf "skip: %s\n" "$f"
      continue
    ...
    if ((DPI > 0)); then
      ...
      pngcrush -brute -l9 -res "$DPI" -s "$f" "$tc" &>/dev/null && [[ -s $tc && $(stat -c%s "$tc") -lt $ns ]] \
        && {
          cp "${p[@]}" "$tc" "$f"
          ((VERB)) && printf 'pngcrush: %s DPI=%s %s→%s\n' "$f" "$DPI" "$ns" "$(stat -c%s "$f")"
        }
      rm -f "$tc"
    fi
...
main() {
  ...
  case $cmd in
    cd)
      [[ $# -eq 1 ]] || die "usage: cd <toc>"
      cd_burn "$@"
      ;;
    usb)
      [[ $# -eq 2 ]] || die "usage: usb <iso> <dev>"
      usb_write "$@"
      ;;
    iso2sd)
      [[ $# -eq 2 ]] || die "usage: iso2sd <iso> <dev>"
      iso2sd "$@"
      ;;
    format)
      [[ $# -eq 2 ]] || die "usage: format <dev> <label>"
      format_exfat "$@"
      ;;
    ripdvd)
      [[ $# -eq 1 ]] || die "usage: ripdvd <iso>"
      rip_dvd "$@"
      ;;
    pngzip) pngzip "$@" ;;
    towebp) towebp "$@" ;;
    vid1080)
      [[ $# -eq 1 ]] || die "usage: vid1080 <video>"
      vid1080 "$@"
      ;;
    vid4k)
      [[ $# -eq 1 ]] || die "usage: vid4k <video>"
      vid4k "$@"
      ;;
    jpg)
      [[ $# -ge 1 ]] || die "usage: jpg <img> [opts]"
      jpg_opt "$@"
      ;;
    jpgsmall)
      [[ $# -ge 1 ]] || die "usage: jpgsmall <img> [opts]"
      jpg_small "$@"
      ;;
    png)
      [[ $# -ge 1 ]] || die "usage: png <img> [opts]"
      png_opt "$@"
      ;;
