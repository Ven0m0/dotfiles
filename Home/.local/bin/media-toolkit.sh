...
cmd_cd() {
  ...
  log "Burning $iso..."
  sudo cdrdao write --eject --driver generic-mmc-raw "$iso"
}
...
cmd_ripdvd() {
  req dd
  local iso=${1:-dvd.iso}
  log "Ripping /dev/sr0 to $iso..."
  sudo dd if=/dev/sr0 of="$iso" bs=2048 status=progress
}
...
cmd_vid() {
  ...
  log "Transcoding $file to $mode..."
  ffmpeg -i "$file" $opts "${file%.*}_${mode}.mp4"
}
