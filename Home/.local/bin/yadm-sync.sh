...
get_repo_dir(){ yadm rev-parse --show-toplevel &>/dev/null && yadm rev-parse --show-toplevel||[[ -d "${HOME}/.local/share/yadm/repo.git" ]] && echo "${HOME}/.local/share/yadm/repo.git"||die "Cannot determine yadm repository location";}
usage(){
  cat <<EOF
...
sync_pull(){
  ...
  log "Syncing FROM repository TO home directory...";log "Source: $home_dir/";log "Target: $HOME/"
...
sync_push(){
  ...
  [[ -d $home_dir ]]||die "Home/ directory not found: $home_dir"
  has rsync||die "rsync required"
  log "Syncing FROM home directory TO repository...";log "Source: $HOME/";log "Target: $home_dir/"
...
EXCLUDES
  ...
  while IFS= read -r -d '' file;do local rel_path="${file#${home_dir}/}" source_file="${HOME}/${rel_path}";[[ -e $source_file ]] && files_to_sync+=("$rel_path");done < <(find "$home_dir" -type f -print0)
  ((${#files_to_sync[@]}>0)) && printf '%s\n' "${files_to_sync[@]}"|rsync "${rsync_opts[@]}" "${HOME}/" "${home_dir}/"
  rm -f "$exclude_file"
  if [[ $dry_run == "1" ]];then warn "DRY RUN - No files modified"
  else ok "Repository updated";log "Next steps:";log "  cd $repo_dir";log "  git add -A";log "  git commit -m 'Update dotfiles'";log "  git push";fi
}
sync_status(){
...
sync_diff(){
  ...
  while IFS= read -r -d '' file;do
    ...
    [[ -f $source_file && -f $file ]] && diff -q "$source_file" "$file" &>/dev/null||{ printf '\n%b%bDifferences in:%b %s\n%b━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━%b\n' "$BLD" "$BLU" "$DEF" "$rel_path" "$BLD" "$DEF";diff -u --color=always "$file" "$source_file" 2>/dev/null||:;}
