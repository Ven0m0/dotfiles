...
https://github.com/Bhupesh-V/.Varshney/blob/master/scripts/shell/surf
...
def list_instances():
  with open(get_cache_path()) as cache:
    for server in cache:
      print(f"{server}", end="")
...
def dont_hurt_instance(query, category):
  ...
  while True:
    ...
    if code == 429:
      ...
      print(f"Oh Snap! {colors.BOLD}{current_instance}{colors.RESET} says {data} :(")
...
def search(query, args):
  ...
  if len(data["corrections"]) == 2:
    print(f"""Did you mean {colors.BOLD}{data["corrections"][1]}{colors.RESET}?\n""")
