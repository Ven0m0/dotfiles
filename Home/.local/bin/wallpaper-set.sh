...
FzfInFolders() {
  cd "$picfolder" || DIE "Cannot access folder: $picfolder"
  local item="$PWD"
  ...
  while true; do
    # Use fd or find instead of ls
    if command -v fd &>/dev/null; then
      mapfile -t entries < <(fd -d 1 --color=always --no-ignore-vcs . "$item")
    else
      mapfile -t entries < <(find "$item" -maxdepth 1 ! -name '.*' -printf '%P\n' | sort)
    fi
...
WallpaperSelect() {
  # Use fd or find to count files
  local pic_count
  if command -v fd &>/dev/null; then
    pic_count=$(fd -t f . "$picfolder" | wc -l)
  else
    pic_count=$(find "$picfolder" -type f | wc -l)
  fi
...
XfceSetWallpaper() {
  ...
  case "$impl" in
    ...
    1)
      ...
      done < <(xfconf-query --channel xfce4-desktop --list | grep /last-image$)
      #eos_yad_infomsg "Reboot is required for the changes to take effect."
      ;;
    2)
      local monitors mon prop
...
PictureNameVariations() {
  ...
  if [[ "$(basename "$pic")" = "$fallback" ]]; then
    local changed
    changed="$(echo "$pic" | sed 's|endeavouros_\([a-z][a-z]*[0-9]\.png\)$|endeavouros-\1|')"
    if [[ -r "$changed" ]]; then
...
RemoveBadWallsFromHistory() {
  ...
  if [[ "${#walls_remove[@]}" -gt 0 ]]; then
    local reply
    read -rp "==> Remove non-existing wallpaper paths from the history (Y/n)? " reply >&2
    case "$reply" in
      [Nn]*) ;;
      *)
  ...
  for arg in "$@"; do
    case "$arg" in
      ...
      --history)
        ...
        local walls=() wp
        mapfile -t walls < <(grep ^[0-9] "$history_of_backgrounds" | sed -E 's|^[^:]+: (/.*)|\1|' | sort -u)
...
Main() {
  ...
  case "$DE" in
    XFCE) command -v xrandr &>/dev/null || DIE "Sorry, this implementation needs package 'xorg-xrandr' on Xfce." ;;
  esac
