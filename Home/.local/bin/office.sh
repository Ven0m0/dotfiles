...
pdf_lossless() {
  ...
  gs -sDEVICE=pdfwrite -dPDFSETTINGS=/default -dCompatibilityLevel=1.7 -dNOPAUSE -dQUIET -dBATCH -sOutputFile="$o" "$f" 2> /dev/null || return 1
  printf '%s\n' "$o"
}
pdf_lossy() {
  local f=$1 o=${f%.*}-lossy.pdf
  gs -sDEVICE=pdfwrite -dPDFSETTINGS=/ebook -dNOPAUSE -q -dBATCH -dDownsampleColorImages=true -dDownsampleMonoImages=true -dDownsampleGrayImages=true -sOutputFile="$o" "$f" 2> /dev/null || return 1
  printf '%s\n' "$o"
}
validate_office() {
...
main() {
  ...
  case $1 in
    compress)
      [[ $# -lt 2 ]] && die "usage: compress <file> [mode]"
      compress_file "$2" "${3:-deflate}"
      ;;
    batch) batch_compress "${2:-deflate}" "${3:-.}" ;;
    media)
      [[ $# -lt 2 ]] && die "usage: media <file> [mode]"
      repack_media "$2" "${3:-lossless}"
      ;;
    deep)
      [[ $# -lt 2 ]] && die "usage: deep <file> [mode]"
      local m=${3:-deflate} tmp
      tmp=$(compress_file "$2" "$m") && repack_media "$tmp" "$([[ $m == lossy ]] && printf lossy || printf lossless)"
      ;;
    lint) lint_office "${2:-.}" ;;
    strip)
      [[ $# -lt 2 ]] && die "usage: strip <file>"
      strip_metadata "$2"
      ;;
    stats)
      [[ $# -lt 2 ]] && die "usage: stats <file>"
      show_stats "$2"
      ;;
    *)
      usage
