...
die(){ err "$@"; exit 1; }
_ver(){ printf '%b%s%b -- %bv3.0.1%b unified pacman/paru TUI\n' "${BLD}" "${0##*/}" "${DEF}" "${UL}" "${DEF}"; }
_help(){
  cat <<EOF
...
${BLD}KEYS${DEF}   Tab:select Enter:confirm Ctrl-S:info Ctrl-J/K:nav
...
_fzf(){
  ...
  while (($#)); do
    case "$1" in
      ...
      -p) opts+=(--preview "$2" --preview-window=wrap); shift 2 ;;
...
_search(){
  ...
  _getlist "$@" | _fzf -m -h "Ctrl-S:info" -p "bash -c '_info {}'" \
    -b "ctrl-s:execute(bash -c '_info {} | less -R')"
}

_local(){
  export -f _infoq _fzf; export PAC FND; declare -gA _CACHE_INFOQ
  _getlocal "$@" | _fzf -m -h "Ctrl-S:info" -p "bash -c '_infoq {}'" \
    -b "ctrl-s:execute(bash -c '_infoq {} | less -R')"
}

_orphans(){
  export -f _infoq _fzf; export PAC FND; declare -gA _CACHE_INFOQ
  "${PAC}" -Qdttq "$@" 2>/dev/null | _fzf -m -h "Ctrl-S:info" -p "bash -c '_infoq {}'" \
    -b "ctrl-s:execute(bash -c '_infoq {} | less -R')"
}
...
# TODO: add "https://github.com/basecamp/omarchy/blob/master/bin/omarchy-pkg-install" functionality
fzf_args=(
  -m --color 'pointer:green,marker:green'
