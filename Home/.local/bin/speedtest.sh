...
set -u
: "${CURL:=$(command -v curl)}" || { printf '%s\n' "curl required"; exit 1; }
...
for srv in "${SERVERS[@]}"; do
  ...
  if [ "$cmp" -eq 1 ]; then best_lat=$avg; best_srv=$srv; fi
done
...
DL_OUT=$("$CURL" -s -L -o /dev/null -w '%{size_download} %{time_total} %{speed_download}' "$DL_URL") || { printf 'Download failed\n'; exit 1; }
...
UL_OUT=$(dd if=/dev/zero bs=1M count="$UL_MB" 2>/dev/null | \
  "$CURL" -s -X POST --data-binary @- -H 'Content-Type: application/octet-stream' -w '%{size_upload} %{time_total} %{speed_upload}' -o /dev/null "$UPLOAD_URL") || { printf 'Upload failed\n'; exit 1; }
