...
sysz_cmd(){
  ...
  local PROG="sysz" VER=1.4.3 VERBOSE=false HIST="${SYSZ_HISTORY:-${XDG_CACHE_HOME:-$HOME/.cache}/sysz/history}" -a MANAGERS STATES
  ((EUID==0)) && MANAGERS=(system)||MANAGERS=(user system)
  while [[ $# -gt 0 ]];do case "$1" in -u|--user) MANAGERS=(user);shift;;--sys|--system) MANAGERS=(system);shift;;-s|--state) STATES+=("--state=$2");shift 2;;--state=*) STATES+=("$1");shift;;-V|--verbose) VERBOSE=true;shift;;-v|--version) printf '%s %s\n' "$PROG" "$VER";return 0;;-h|--help) sysz_help;return 0;;*) break;;esac;done
  local CMD="" -a EXTRA
  while [[ $# -gt 0 ]];do case "$1" in _fzf_preview) shift;sysz_preview "$@";return;;_fzf_cat) shift;sysz_cat "$@";return;;--) shift;EXTRA=("$@");break;;re) CMD=restart;shift;;s) CMD=status;shift;;ed) CMD=edit;shift;;en) CMD=enable;shift;;d|dis) CMD=disable;shift;;j) CMD=journal;shift;;f) CMD=follow;shift;;c) CMD='cat';shift;;*) CMD="$1";shift;;esac;done
  mkdir -p "$(dirname "$HIST")";: >"$HIST"||:
  local ST;for ST in "${STATES[@]}";do ST="${ST##*=}";[[ -n $ST ]] && systemctl --state=help|grep -Fx "$ST" &>/dev/null||die "bad state: $ST";done
  local -a UNITS KEY
  while :;do
    ...
    ((${#UNITS[@]}))||return 1;KEY="${UNITS[0]}";UNITS=("${UNITS[@]:1}")
    case "$KEY" in ctrl-r) sysz_daemon_reload;continue;;ctrl-s) sysz_pick_states STATES;continue;;esac
    ((${#UNITS[@]}))||return 1;break
  done
  local -a CMDS
  [[ -n $CMD ]] && CMDS=("$CMD")||{ CMDS=("$(sysz_pick_commands "${UNITS[@]}" "${EXTRA[@]}")");((${#CMDS[@]}))||return 1;}
  local U M UNIT CODE=0
  for U in "${UNITS[@]}";do M=$(sysz_manager "$U");UNIT="${U##* }";for CMD in "${CMDS[@]}";do sysz_exec "$M" "$UNIT" "$CMD" "${EXTRA[@]}"||CODE=$?;[[ ${#UNITS[@]} -eq 1 ]] && return "$CODE";done;done
}
sysz_manager(){ case ${1%% *} in '[user]') printf -- '--user';;'[system]') printf -- '--system';;*) die "mgr";;esac;}
sysz_cat(){ local M=$(sysz_manager "$1") U="${1##* }";SYSTEMD_COLORS=1 systemctl "$M" cat -- "$U";}
sysz_preview(){ local M=$(sysz_manager "$1") U="${1##* }";[[ $U == *@.* ]] && sysz_cat "$@"||SYSTEMD_COLORS=1 systemctl "$M" status --no-pager -- "$U";}
sysz_show(){ systemctl "$1" show "$2" -p "$3" --value;}
sysz_sort(){
  local line mgr unit uclean n type
  while IFS= read -r line;do mgr=${line%% *};unit=${line##* };uclean=${unit//$'\e'[\[(]*([0-9;])[@-n]/}
...
sysz_list(){
  ...
  (systemctl list-units "${args[@]}";systemctl list-unit-files "${args[@]}")|sort -u -t ' ' -k1,1|while read -r l;do local unit=${l%% *}
...
sysz_list_units(){
  local -a mgrs=("$@") states=() x
  for x in "${mgrs[@]}";do [[ $x == --state=* ]] && states+=("$x");done
  local -a mm;for x in "${mgrs[@]}";do [[ $x != --state=* ]] && mm+=("${x#--}");done
  local M;for M in "${mm[@]}";do sysz_list "--$M" "${states[@]}"|sed -E "s/^/[$M] /";done|sysz_sort
}
sysz_daemon_reload(){
  local picks=$(printf '%s\n' '[system] daemon-reload' '[user] daemon-reload'|fzf --multi --no-info --prompt='Reload: ')||return 1
  [[ -z $picks ]] && return 1;local line
  while IFS= read -r line;do case "$line" in '[user] daemon-reload') systemctl --user daemon-reload;;'[system] daemon-reload') ((EUID==0)) && systemctl --system daemon-reload||sudo systemctl --system daemon-reload;;esac;done <<<"$picks"
}
sysz_pick_states(){
  local -n ref="$1"
  mapfile -t chosen < <(systemctl --state=help|grep -Ev ':|ing|^$'|sort -u|fzf --multi --prompt='States: ')||return 1
  ((${#chosen[@]}))||return 0;ref=();local st;for st in "${chosen[@]}";do ref+=("--state=$st");done
}
sysz_pick_commands(){
  local -a units=() extra=()
  while [[ $1 != "" && $1 != "--" ]];do units+=("$1");shift;done;shift||:;extra=("$@")
  local MULTI=false ACTIVE LOAD UF CAN U PREVIEW PREVIEW_CMD
  if ((${#units[@]}>1));then MULTI=true;PREVIEW="$(printf '%s\n' "${units[@]}")";PREVIEW_CMD="echo -n '$PREVIEW'"
  else
    U="${units[0]}";[[ $U == *@.* ]] && read -r -p "$U param: " PARAM && U="${U/@/@$PARAM}" && units[0]="$U"
  ...
  mapfile -t cmds < <(
    ...
    echo journal "${extra[*]}";echo follow "${extra[*]}"
    ...
    echo cat "${extra[*]}";echo edit "${extra[*]}";echo show "${extra[*]}"
  )
  mapfile -t pick < <(printf '%s\n' "${cmds[@]}"|fzf --multi --ansi --no-info --prompt='Commands: ' --preview="$PREVIEW_CMD" --preview-window=80%)
  printf '%s\n' "${pick[@]}"
}
sysz_exec(){
  local M="$1" UNIT="$2" CMD="$3";shift 3;local -a args=("$@") run=(systemctl "$M") base="${CMD%% *}"
  case "$base" in
    ...
    mask|unmask|start|stop|restart|reload|enable|disable) "${run[@]}" "$CMD" "${args[@]}" -- "$UNIT"||return $?;SYSTEMD_COLORS=1 "${run[@]}" status --no-pager -- "$UNIT";return;;
    *) "${run[@]}" "$CMD" "${args[@]}" -- "$UNIT";return;;
  esac
}
sysz_journal(){ local M="$1" UNIT="$2";shift 2;[[ $M == --user ]] && journalctl --user-unit="$UNIT" "$@"||{ ((EUID!=0)) && sudo journalctl --unit="$UNIT" "$@"||journalctl --unit="$UNIT" "$@";};}
prsync_usage(){ printf 'Usage: sysmaint prsync [--parallel=N] RSYNC_ARGS...\n';}
prsync_cmd(){
  (($#))||{ prsync_usage;return 2;}
  local par="";[[ $1 == --parallel=* ]] && { par="${1##*=}";shift;}
  [[ -n $par ]]||par="$(nproc 2>/dev/null||printf 10)";log "parallel=$par"
  ...
  local total=$(wc -l <"$TMP/all") size=$(awk '{s+=$1}END{printf "%.0f",s}' <"$TMP/all")
  log "$total files ($((size/1024**2)) MB)";((total))||{ warn none;return 0;}
  local i;for ((i=0;i<par;i++));do : >"$TMP/chunk.$i";done
  local -a sum;for ((i=0;i<par;i++));do sum[i]=0;done
  local sz path idx=0
  while read -r sz path;do
    local mi=0 mv=${sum[0]} j;for ((j=1;j<par;j++));do ((${sum[j]}<mv)) && { mv=${sum[j]};mi=$j;};done
    sum[mi]=$((sum[mi]+sz));printf '%s\n' "$path" >>"$TMP/chunk.$mi";((++idx%25000==0)) && log "distributed $idx"
  done <"$TMP/all"
  for ((i=1;i<par;i+=2));do [[ -s "$TMP/chunk.$i" ]] && tac "$TMP/chunk.$i" >"$TMP/r" && mv "$TMP/r" "$TMP/chunk.$i"||:;done
  log transferring
  has parallel && find "$TMP" -name 'chunk.*' -print0|parallel -0 -j "$par" -t -- rsync --files-from={} "$@"||has xargs && find "$TMP" -name 'chunk.*' -print0|xargs -0 -n1 -P"$par" -I{} rsync --files-from={} "$@"||while read -r f;do rsync --files-from="$f" "$@";done < <(find "$TMP" -name 'chunk.*')
}
main(){
  local c="${1:-}";shift||:
  case "$c" in ln2) ln2_cmd "$@";;swap) swap_cmd "$@";;symclean) symclean_cmd "$@";;usb) usb_cmd "$@";;sysz) sysz_cmd "$@";;prsync) prsync_cmd "$@";;""|-h|--help|help) usage;;*) die "unknown subcmd: $c";;esac
}
main "$@"
