...
ln2_cmd(){
  ...
  local op1 op op2; op2=${a[-1]}; op=${a[-2]}; op1=${a[-3]}
  a=("${a[@]:0:${#a[@]}-3}")
...
swap_cmd(){
  ...
  local a="$1" b="$2" tmp
  if [[ -d "$a" ]]; then tmp="tmp.$$.dir"; mkdir -p "$tmp"; else tmp="$(mktemp)"; fi
...
usb_cmd(){
  ...
  local i=0; local -a names uuids fss labels mpts
  ...
  if [[ -z "$mp" ]]; then
    ...
    for ((k=1;k<=cnt;k++)); do
      ...
      local used=0
      for mp in "${mpts[@]}"; do [[ "$mp" == "$candidate" ]] && { used=1; break; }; done
...
sysz_cat(){ local M=$(sysz_manager "$1"); local U="${1##* }"; SYSTEMD_COLORS=1 systemctl "$M" cat -- "$U"; }
sysz_preview(){
  local M=$(sysz_manager "$1"); local U="${1##* }"
...
sysz_list(){
  local -a args; args=(--all --no-legend --full --plain --no-pager "$@")
  ( systemctl list-units "${args[@]}"; systemctl list-unit-files "${args[@]}" ) | \
    sort -u -t ' ' -k1,1 | while read -r l; do
      local unit=${l%% *}
      if [[ $l == *" active "* ]]; then printf '\033[0;32m%s\033[0m\n' "$unit"
      elif [[ $l == *" failed "* ]]; then printf '\033[0;31m%s\033[0m\n' "$unit"
      elif [[ $l == *" not-found "* ]]; then printf '\033[1;33m%s\033[0m\n' "$unit"
      else printf '%s\n' "$unit"
      fi
    done
}
...
sysz_list_units(){
  local -a mgrs states; mgrs=("$@"); states=()
  # separate states
  local x; for x in "${mgrs[@]}"; do [[ "$x" == --state=* ]] && states+=("$x") ; done
...
sysz_pick_states(){
  ...
  ((${#chosen[@]})) || return 0
  ref=()
  local st; for st in "${chosen[@]}"; do ref+=("--state=$st"); done
}
...
sysz_exec(){
  local M="$1" UNIT="$2" CMD="$3"; shift 3
  local -a args; args=("$@")
  local base="${CMD%% *}"
...
sysz_journal(){
  local M="$1" UNIT="$2"; shift 2
  if [[ "$M" == --user ]]; then journalctl --user-unit="$UNIT" "$@"
  else
...
prsync_cmd(){
  ...
  local total; total=$(wc -l <"$TMP/all")
  local size; size=$(awk '{s+=$1}END{printf "%.0f",s}' <"$TMP/all")
  ...
  while read -r sz path; do
    ...
    for ((j=1;j<par;j++)); do (( ${sum[j]} < mv )) && { mv=${sum[j]}; mi=$j; }; done
    sum[mi]=$(( sum[mi]+sz ))
    printf '%s\n' "$path" >>"$TMP/chunk.$mi"
