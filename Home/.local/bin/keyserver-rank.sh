...
rank_server(){
  ...
  time=$(TIMEFORMAT='%3R'; { time gpg --batch --keyserver "$srv" --search-keys info@endeavouros.com &>/dev/null; } 2>&1)
  code=$?
  if [[ $code -eq 0 ]]; then
...
main(){
  ...
  while (($#)); do
    case $1 in
      --yes|-y) mode=yes;;
      --no|-n) mode=no;;
      --show-fastest) mode=no; show_fastest=yes;;
      --list-servers) printf '%s\n' "${servers[@]}"; exit 0;;
