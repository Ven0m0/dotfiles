    ...
    def connect(self):
      ...
      if ssl:
        try:
          ...
          # For older Python versions without SSLContext,
          # use ssl.wrap_socket with certificate verification
          server_hostname = self._tunnel_host if self._tunnel_host else self.host
          try:
            # Try to use certificate verification if available
            import ssl as ssl_module

            if hasattr(ssl_module, "CERT_REQUIRED"):
              ...
              if hasattr(ssl_module, "PROTOCOL_TLSv1_2"):
                ...
                raise SpeedtestException(
                  "Unable to establish a secure SSL/TLS connection: No TLSv1.2+ protocol available in ssl module."
                )
          except (AttributeError, TypeError):
...
def build_opener(source_address=None, timeout=10):
  ...
  if ssl and hasattr(ssl, "create_default_context"):
    try:
      ...
      https_context.check_hostname = True
  ...
  handlers = [
    ...
    SpeedtestHTTPSHandler(source_address=source_address_tuple, timeout=timeout, context=https_context),
    HTTPDefaultErrorHandler(),
    HTTPRedirectHandler(),
    HTTPErrorProcessor(),
  ]
...
def shell():
  ...
  printer(
    "Hosted by %(sponsor)s (%(name)s) [%(d)0.2f km]: %(latency)s ms" % results.server,
    quiet,
  )
  if args.download:
    ...
    printer(
      "Download: %0.2f M%s/s" % ((results.download / 1000.0 / 1000.0) / args.units[1], args.units[0]),
      quiet,
    )
  else:
  ...
  if args.upload:
    ...
    printer(
      "Upload: %0.2f M%s/s" % ((results.upload / 1000.0 / 1000.0) / args.units[1], args.units[0]),
      quiet,
    )
  else:
