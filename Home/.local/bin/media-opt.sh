...
opt_img(){
  ...
  # 2. image-optimizer (Rust wrapper)
    ...
    tool="image-optimizer"; cmd=(image-optimizer "$f" "$out")
  # 3. Fallbacks
  else
    case "$l_ext" in
      jpg|jpeg|mjpg) has jpegoptim && { tool="jpegoptim"; cmd=(jpegoptim --strip-all --all-progressive --stdout "$f"); [[ $LOSSLESS -eq 0 ]] && cmd+=(-m"$QUALITY"); } ;;
      png) has oxipng && { tool="oxipng"; cmd=(oxipng -o 4 --strip safe -i 0 --out - "$f"); } ;;
      webp) has cwebp && { tool="cwebp"; cmd=(cwebp -mt -quiet "$f" -o -); [[ $LOSSLESS -eq 1 ]] && cmd+=(-lossless -z 9) || cmd+=(-q "$QUALITY" -m 6); } ;;
    esac
  fi
}
...
opt_svg(){
  local f="$1" out="$2"
  if has image-optimizer; then tool="image-optimizer"; cmd=(image-optimizer "$f" "$out")
  elif has svgcleaner; then tool="svgcleaner"; cmd=(svgcleaner "$f" "$out")
  elif has scour; then tool="scour"; cmd=(scour -i "$f" -o "$out" --enable-viewboxing --enable-id-stripping --shorten-ids --indent=none)
  elif has minify; then tool="minify"; cmd=(minify -o "$out" "$f")
  elif has svgo; then tool="svgo"; cmd=(svgo -i "$f" -o - --multipass)
  fi
}

opt_vid(){
  local f="$1" out="$2"
  local a_args=(-c:a "$AUDIO_CODEC" -b:a "$AUDIO_BR"); [[ "$AUDIO_CODEC" == "copy" ]] && a_args=(-c:a copy)
  
  if has ffzap; then tool="ffzap"; cmd=(ffzap -i "$f" -o "$out")
  elif has ffmpeg; then
    tool="ffmpeg"; local v_args=()
    case "$VIDEO_CODEC" in
...
optimize_file(){
  ...
  local ext="${f##*.}"; local tmp="${f}.opt.tmp.${ext}"; local tool="" cmd=()

  case "${ext,,}" in
    jpg|jpeg|mjpg|png|webp|avif|jxl|bmp) opt_img "$f" "$tmp" ;;
    svg) opt_svg "$f" "$tmp" ;;
    gif) has gifsicle && { tool="gifsicle"; cmd=(gifsicle "$f"); [[ $LOSSLESS -eq 1 ]] && cmd+=(-O3 --careful) || cmd+=(-O3 --lossy=80); } ;;
    mp4|mkv|mov|avi|webm) opt_vid "$f" "$tmp" ;;
    *) return 0 ;;
  esac

  [[ -z "$tool" ]] && { [[ -f "$tmp" ]] && rm -f "$tmp"; return 0; }
  
  if [[ "$DRY_RUN" -eq 1 ]]; then
    printf "${B}[DRY]${X} %-10s %s\n" "$tool" "$f"; [[ -f "$tmp" ]] && rm -f "$tmp"; return 0
  fi
  ...
  if [[ $ok -eq 1 ]] && [[ -f "$tmp" ]]; then
    ...
    if [[ $new_sz -gt 0 ]] && [[ $new_sz -lt $old_sz ]]; then
      ...
      if [[ "$BACKUP" -eq 1 ]]; then
        local bp="${BACKUP_DIR}/${f#.}"; mkdir -p "$(dirname "$bp")"; cp -p "$f" "$bp"
      fi
      mv "$tmp" "$f"; [[ "$KEEP_MTIME" -eq 1 ]] && touch -r "$f" "$f"
...
[[ "$BACKUP" -eq 1 ]] && { mkdir -p "$BACKUP_DIR"; log "Backup: $BACKUP_DIR"; }

log "Starting ($JOBS jobs, $VIDEO_CODEC)..."
EXTS="jpg,jpeg,mjpg,png,webp,svg,gif,avif,jxl,bmp,mp4,mkv,mov,webm,avi"

if has fd; then FIND="fd -t f -H -E .git $(printf -- "-e %s " ${EXTS//,/ }) . \"$@\""
elif has fdfind; then FIND="fdfind -t f -H -E .git $(printf -- "-e %s " ${EXTS//,/ }) . \"$@\""
else FIND="find \"$@\" -type f -iregex \".*\.\(${EXTS//,/\|}\)$\" -not -path '*/.*'"; fi

if has rust-parallel; then eval "$FIND" | rust-parallel -j "$JOBS" -- 'optimize_file {}'
elif has parallel; then eval "$FIND" | parallel -j "$JOBS" --no-notice "optimize_file {}"
else eval "$FIND -print0" | xargs -0 -P "$JOBS" -I {} bash -c 'optimize_file "$@"' _ {}
fi
log "Done."
