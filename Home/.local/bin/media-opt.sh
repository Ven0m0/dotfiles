...
# ==============================================================================
interactive_mode(){
  has ffmpeg || { err "ffmpeg required for interactive mode"; exit 1; }
  while :; do
    clear
    ...
    input_path="${input_path#\"}"
    input_path="${input_path//\'/}"
    [[ ! -f $input_path ]] && { printf '%s\n' "${R}File not found${X}"; continue; }
