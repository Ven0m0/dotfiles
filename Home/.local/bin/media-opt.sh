...
declare -A TC
cache_tools(){ local t; for t in rimage jpegoptim oxipng cwebp gifsicle ffmpeg; do has "$t"&&TC[$t]=1||TC[$t]=0; done; }
hc(){ [[ ${TC[$1]:-0} -eq 1 ]]; }
opt_img(){
  ...
  if hc rimage; then
    ...
    case "$lx" in
      jpg|jpeg|mjpg)
        hc jpegoptim&&{ t=jpegoptim; c=(jpegoptim --strip-all --all-progressive --stdout "$f"); [[ $LOSSLESS -eq 0 ]]&&c+=(-m"$QUAL"); };;
      png) hc oxipng&&{ t=oxipng; c=(oxipng -o4 --strip safe -i0 --out - "$f"); };;
      webp)
        hc cwebp&&{ t=cwebp; c=(cwebp -mt -quiet "$f" -o -); [[ $LOSSLESS -eq 1 ]]&&c+=(-lossless -z9)||c+=(-q "$QUAL" -m6); };;
    esac
  fi
  [[ -n $t ]]&&printf '%s:%s\n' "$t" "${c[*]}"
}
...
optimize(){
  ...
  if [[ $ok -eq 1 && -f $tmp ]]; then
    local os ns d p; os=$(stat -c%s "$f"); ns=$(stat -c%s "$tmp")
...
interactive(){
  ...
  while :; do
    ...
    if [[ $lext == gif && $cext != gif ]]; then
      ...
      [[ $cext == gif ]]&&vff="fps=30${vff:+,${vff}}"; [[ -n $vff ]]&&cmd+=(-vf "$vff")
    fi
