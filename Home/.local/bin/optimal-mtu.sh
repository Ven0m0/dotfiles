...
select_iface() {
  local -a list
  mapfile -t list < <(ip -br link | awk '$1!~/(lo|veth|docker|br-)/{print $1}')
...
persist() {
  ...
  if command -v nmcli > /dev/null && nmcli -t dev | grep -q "^${iface}:"; then
    local c
    c=$(nmcli -t -f NAME,DEVICE con show --active | awk -F: -v i="$iface" '$2==i{print $1}')
    [[ -n $c ]] && {
      sudo nmcli con mod "$c" 802-3-ethernet.mtu "$mtu"
      echo "Saved via NetworkManager"
      return
    }
  fi
  ...
  if [[ -d /etc/systemd/network ]]; then
    ...
    echo "Saved to /etc/systemd/network/ (restart systemd-networkd to apply)"
    return
  fi
  ...
  if [[ -f /etc/network/interfaces ]]; then
    if grep -q "iface $iface inet" /etc/network/interfaces; then
      ...
      echo "Saved to /etc/network/interfaces"
      return
    fi
  fi
...
find_mtu() {
  ...
  local old_mtu
  old_mtu=$(ip -o link show "$iface" | awk '{print $5}')
...
main() {
  ...
  read -rp "Apply permanent? (y/N) " -n1 choice
  echo
  [[ $choice =~ ^[Yy]$ ]] || exit 0
