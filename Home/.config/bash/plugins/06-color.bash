...
#–– Colors
if has vivid; then
  export VIVID_THEME="dracula" LS_COLORS="$(vivid generate dracula)"
elif has dircolors; then
  eval "$(dircolors -b &>/dev/null)" || true
else
...
export EZA_COLORS="$LS_COLORS" EZA_ICONS_AUTO=1 CLICOLOR=1 SYSTEMD_COLORS=1 PYTHON_COLORS=1; unset NOCOLOR
...
C(){ printf '\e[%d;5;%sm' "$([[ $1 == bg_* ]] && printf "48" || printf "38")" "${1#*_}"; }
