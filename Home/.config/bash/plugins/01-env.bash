...
if has firefox; then
  export BROWSER='firefox' GTK_USE_PORTAL=1 MOZ_ENABLE_WAYLAND=1 MOZ_DBUS_REMOTE=1 MOZ_ENABLE_XINPUT2=1 MOZ_DISABLE_RDD_SANDBOX=1
else
...
if [[ "${XDG_SESSION_TYPE-}" == "wayland" ]]; then
  ...
  if has nvidia-smi; then
    export NVD_BACKEND=direct LIBVA_DRIVER_NAME=nvidia VDPAU_DRIVER=nvidia __GLX_VENDOR_LIBRARY_NAME=nvidia
    export mesa_glthread=true __GL_THREADED_OPTIMIZATIONS=1 __GL_SHADER_DISK_CACHE_SKIP_CLEANUP=1 __GL_SHADER_DISK_CACHE_SIZE=12000000000
...
# --- Gaming
if has proton; then
  export PROTON_ENABLE_WAYLAND=1 PROTON_NO_WM_DECORATION=1 PROTON_USE_NTSYNC=1 PROTON_PREFER_SDL=1 PROTON_ENABLE_HDR=1
  export PROTON_DLSS_UPGRADE=1 PROTON_FSR4_UPGRADE=1 PROTON_XESS_UPGRADE=1
  export PROTON_NVIDIA_LIBS_NO_32BIT=1 PROTON_NVIDIA_NVENC=1 PROTON_ENABLE_NGX_UPDATER=1 #PROTON_NVIDIA_NVCUDA=1 PROTON_NVIDIA_NVML=1
  export DXVK_NVAPI_DRS_NGX_DLSS_RR_OVERRIDE=on DXVK_NVAPI_DRS_NGX_DLSS_SR_OVERRIDE=on DXVK_NVAPI_DRS_NGX_DLSS_FG_OVERRIDE=on
  export DXVK_NVAPI_DRS_NGX_DLSS_RR_OVERRIDE_RENDER_PRESET_SELECTION=render_preset_latest DXVK_NVAPI_DRS_NGX_DLSS_SR_OVERRIDE_RENDER_PRESET_SELECTION=render_preset_latest
fi
if has wine; then
