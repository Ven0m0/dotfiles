...
export FZF_COMPLETION_OPTS="${FZF_COMPLETION_OPTS:---height=40% --reverse --ansi --prompt='❯ '}"
FZF_SEP=$'\x01'
...
_fzf_selector(){
  local fzf="$(_fzf_cmd)" height="${FZF_TMUX_HEIGHT:-40%}"
  ...
  FZF_DEFAULT_OPTS="--height $height --reverse $FZF_DEFAULT_OPTS $FZF_COMPLETION_OPTS" \
    "$fzf" -1 -0 --prompt "$FZF_COMPLETION_PROMPT" --nth=2 --with-nth=2,3 -d "$FZF_SEP" --ansi \
  | cut -d "$FZF_SEP" -f1
}
...
_fzf_completer(){
  ...
  ) | sort -u | _fzf_colorize | _fzf_selector "$1" "$2" "$3" )
  [[ -n $result ]] || return 1
  result="${result// /}"
  COMPREPLY="$result"
